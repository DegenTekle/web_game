...
server = "172.30.11.83" #inserting IP address, the server address, type "ipconfig"  in terminal -> it's the IPv4
port = 1111 #it's a typical port that's open, it's going to look for certain connections
...
s.listen() #listening for connections, it opens up the port so that we can start connecting to it and having multiple clients, if no parameters unlimited connections allowed
        ...
        try:
            ...
            if gameId in games: #every time we run the while loop we're going to check if the game still exists, to see if the gameId which is the key TO access the game, because if one of our clients disconnect from the game we are going to delete that game
                ...
                if not data:
                    ...
                    reply = game #we're going to constantly send back the game object
                    conn.sendall(pickle.dumps(reply)) #this is going to package up our game into a sendable form, we're going to send it to our client, which will receive it, unpickle it, and then use it
            else:
                break
